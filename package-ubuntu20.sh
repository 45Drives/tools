...
if [[ "$(docker images -q $PACKAGE_NAME-ubuntu-builder-ubuntu20 2> /dev/null)" == "" ]]; then
	docker build -t $PACKAGE_NAME-ubuntu-builder-ubuntu20 - < docker/ubuntu20 # pass in path to docker file
	res=$?
	if [ $res -ne 0 ]; then
