...
setBAYstatus() {
        BAYSTATUS[$1]=$(printf "$DGREY%-6s$NC" ${BAY[$1]} "-")
	LINUX_NAME=$(readlink -f $DEVICE_PATH/${BAY[$1]})
        partcount=$((ls $DEVICE_PATH/ | grep -w ${BAY[$1]} | wc -l) 2>/dev/null)
        
        if [ "$partcount" -eq "1" ];then
                BAYSTATUS[$1]=$(printf "$YELLOW%-6s$NC" ${BAY[$1]} "("$LINUX_NAME")")
        elif [ "$partcount" -gt "1" ];then
                BAYSTATUS[$1]=$(printf "$GREEN%-6s$NC" ${BAY[$1]} "("$LINUX_NAME")")
        fi
}
setBAYstatusNC() {
        BAYSTATUS[$1]=$(printf "%-5s" ${BAY[$1]})
        LINUX_NAME=$(readlink -f $DEVICE_PATH/${BAY[$1]})
        partcount=$((ls $DEVICE_PATH/ | grep -w ${BAY[$1]} | wc -l) 2>/dev/null)
        
        if [ "$partcount" -eq "1" ];then
                BAYSTATUS[$1]=$(printf "*%-5s" ${BAY[$1]} "("$LINUX_NAME")")
        elif [ "$partcount" -gt "1" ];then
                BAYSTATUS[$1]=$(printf "**%-5s" ${BAY[$1]} "("$LINUX_NAME")")
        fi
}
